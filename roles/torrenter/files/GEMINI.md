...
- An `.env` file must be present in this directory with the necessary secrets (`WIREGUARD_PRIVATE_KEY`, `TRANSMISSION_PASS`, `PTP_USER`, `PTP_KEY`, `MAM_ID`, `GLUETUN_USER`, `GLUETUN_PASS`).

### Running the Stack
To build and run the entire stack in detached mode:
```bash
