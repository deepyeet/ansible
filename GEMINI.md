...
| **`user_friendly`** | Local | **System-wide Experience Layer.** Installs a static list of common, user-agnostic terminal packages (htop, curl, eza, fd-find, ripgrep, zoxide), build tools (`build-essential`, `cmake`), and debug tools (`gdb`, `strace`). It also installs `atuin` from a pre-compiled binary and sets the system timezone, acting as a baseline for an interactive system. |
